...
namespace mailwizz\Endpoint;

use mailwizz\MailWizzApiBase;
use mailwizz\Http\Client as myClient;

class Campaigns extends MailWizzApiBase
{
    /**
    ...
    {
        $client = new myClient(array(
            'method'        => myClient::METHOD_GET,
     ...
     * 
     * @param string $campaignUid
     * @return MailWizzApi_Htt p_Response
     */
    public function markSent($campaignUid)
    {
        //var_dump($this->config->getApiUrl(sprintf('campaigns/%s/mark-sent', $campaignUid))); die;
        $client = new Client(array(
            'method'    => Client::METHOD_PUT,
